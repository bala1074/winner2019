...
public class Day6_June2_2019 {
	...
	public static void updateLazy(int arr[], int tree[], int lazy[], int treeNode, int s, int e, int l, int r,
		...
		int m = (s + e) / 2;
		updateLazy(arr, tree, lazy, treeNode * 2, s, m, l, r, val);
		updateLazy(arr, tree, lazy, treeNode * 2 + 1, m + 1, e, l, r, val);
		tree[treeNode] = tree[treeNode * 2] + tree[treeNode * 2 + 1];
	}
	...
	public static void dfs2(LinkedList<Integer>[] graph, int src, HashSet<Integer> isv, HashSet<Integer> com) {
		...
		for (int i = 0; i < graph[src].size(); i++) {
			if (!isv.contains(graph[src].get(i)))
				dfs2(graph, graph[src].get(i), isv, com);
		}
	}
	...
	public static LinkedList<HashSet<Integer>> cc(LinkedList<Integer>[] graph, int n) {
		...
		LinkedList<HashSet<Integer>> cc = new LinkedList<HashSet<Integer>>();
		for (int i = 1; i <= n; i++) {
			if (!isv.contains(i)) {
	...
	public static LinkedList<HashSet<Integer>> scc(LinkedList<Integer>[] graph, LinkedList<Integer>[] tgraph, int n) {
		...
		LinkedList<HashSet<Integer>> scc = new LinkedList<HashSet<Integer>>();
		isv = new HashSet<>();
