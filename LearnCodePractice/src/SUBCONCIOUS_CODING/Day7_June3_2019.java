...
public class Day7_June3_2019 {
	...
	public static void update(int arr[], int tree[], int treeNode, int s, int e, int ind, int val) {
		...
		int m = (s + e) / 2;
		if (ind > m)
			update(arr, tree, treeNode * 2 + 1, m + 1, e, ind, val);
		else
			update(arr, tree, treeNode * 2, s, m, ind, val);
		tree[treeNode] = tree[treeNode * 2] + tree[treeNode * 2 + 1];
	}
	...
	public static int exp(int base, int pow) {
		int res = exp(base, pow / 2);
		return pow % 2 == 0 ? res * res : res * res * base;
	}
	...
	public static void bfs(LinkedList<Integer>[] graph, int src) {
		...
		while (!q.isEmpty()) {
			...
			for (int i = 0; i < graph[n].size(); i++) {
				if (!isv.contains(graph[n].get(i))) {
					q.add(graph[n].get(i));
					isv.add(graph[n].get(i));
				}
			}
		}
	}
